...
class DockerBuildStage implements Serializable {
    ...
    DockerBuildStage(script, String registry, String containerName, String dockerImageName, String dockerImageTag) {
        this.script = script
    ...
    def run() {
        script.withCredentials([script.string(credentialsId: 'docker-access-token', variable: 'DOCKER_ACCESS_TOKEN')]) {
            ...
            script.sh "docker rm -f ${containerName} 2>/dev/null || true"
